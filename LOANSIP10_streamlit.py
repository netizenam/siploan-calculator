"""
LOANSIP10_streamlit.py — SIP Loan Calculator (Streamlit)
Coli 23: Budget-Neutral Planner moved to full-width section below both columns
Coli 24: Header made compact — smaller padding and font
All Coli 1-22 retained.
"""
...
<style>
    ...
    .tip-blue {
        ...
        font-size: 0.8rem; color: #1E40AF; margin-bottom: 0.4rem;
    }
    .tip-green {
        background: #F0FDF4; border: 1px solid #BBF7D0;
        border-radius: 6px; padding: 0.4rem 0.8rem;
        font-size: 0.8rem; color: #166534; margin-bottom: 0.4rem;
    }
    .opt-section {
    ...
    .opt-section-title {
        color: #059669; font-weight: 700; font-size: 1rem; margin-bottom: 0.2rem;
    }
    .opt-section-sub {
        color: #166534; font-size: 0.8rem; margin-bottom: 0.6rem;
    }
    .full-divider {
        border: none; border-top: 2px solid #E2E8F0; margin: 1.2rem 0 0.8rem;
    }
...
def grid6(tiles):
    return f'<div class="tile-grid-6">{"".join(tiles)}</div>'


# ─────────────────────── HEADER (Coli 24: compact) ───────────────────────
...
with left:
    if error_msg:
        ...
        st.markdown('<p class="section-title-blue" style="margin-top:0.6rem">Result</p>', unsafe_allow_html=True)
        st.markdown(grid([
    ...
    if not loan_data:
        ...
        if closure_month:
            ...
            st.caption(
                ...
                f"your corpus matches the outstanding balance at Month {closure_month} (Year {closure_year:.1f}), "
            ...
            st.caption("'SIP corpus if held full term' = SIP value if continued for the full original tenure without early closure.")
        else:
            ...
            st.caption("'SIP corpus if held full term' = SIP value if continued for the full original tenure.")

# ═══════════════════════════════════════════
#  FULL WIDTH — BUDGET-NEUTRAL PLANNER (Coli 23)
# ═══════════════════════════════════════════
loan_data = st.session_state.get("loan_result", None)
    ...
    if budget_str.strip():
        try:
            ...
            if opt_loan <= 0 or opt_emi <= 0:
                st.warning("Budget too low for this ROI and tenure combination. Please try a higher amount.")
            else:
                ...
                # 6 tiles across full width
                st.markdown(grid6([
                    tile("Recommended loan",    fmt(opt_loan),                                          "val-green", opt=True),
